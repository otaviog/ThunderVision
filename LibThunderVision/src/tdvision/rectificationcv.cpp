...
bool RectificationCV::update()
{
    ...
    if ( m_rlpipe->read(&limg) && m_rrpipe->read(&rimg) )
    {
        ...
        double _iM[3][3], _R1[3][3], _R2[3][3];
        ...
        CvMat *mxLeft = cvCreateMat(maxheight,  maxwidth, CV_32F),
            *myLeft = cvCreateMat(maxheight,  maxwidth, CV_32F),
            *mxRight = cvCreateMat(maxheight,  maxwidth, CV_32F),
            *myRight = cvCreateMat(maxheight,  maxwidth, CV_32F);
        ...
        IplImage *lo = cvCreateImage(cvGetSize(limg8bit_c),
                                     IPL_DEPTH_8U, 1);
        IplImage *ro = cvCreateImage(cvGetSize(limg8bit_c),
                                     IPL_DEPTH_8U, 1);
        
        IplImage *limout_c = limout.cpuMem();
        IplImage *rimout_c = rimout.cpuMem();
        
        cvShowImage("O", limg_c);
