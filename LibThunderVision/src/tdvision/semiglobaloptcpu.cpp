...
static void costVolume(const Dim &dsiDim,
                       ...
                       float *aggregVol)
{
    float lastCost[512][128];
    float newCost[512][128];
    ...
    for (int x=1; x < static_cast<int>(scanDim.width()); x++)
    {
        for (int y=0; y < static_cast<int>(scanDim.height()); y++)
        {
            ...
            {
                minDisp = std::min(lastCost[y][i], minDisp);
            }
            ...
            for (int d=0; d<static_cast<int>(dsiDim.depth()); d++)
            {
                ...
                const float lcDm1 = d > 0 ? lastCost[y][d - 1] : 0.0f;
                const float lcDp1 = d < static_cast<int>((dsiDim.depth() - 1)) ? lastCost[y][d + 1] : 0.0f;
                ...
                float *dispImg)
{
    for (size_t y=0; y<dsiDim.height(); y++)
    {
        for (size_t x=0; x<dsiDim.width(); x++)
        {
            ...
            dispImg[dsiDim.width()*y + x] = float(minDisp)/float(dsiDim.depth());
        }
    }
}
